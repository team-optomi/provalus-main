...
class CareerScrollingBlog extends Component {
              ...
              <Slider {...settings}>
                ...
                <BlogSlide>
                    ...
                    <Img fluid={imageSrc.node.featuredImage.node.localFile.childImageSharp.fluid} alt={imageSrc.node.featuredImage.node.title} />
                        <Link to={`/career-blog/${imageSrc.node.slug}`}>
                            <div class="title-content">
                                <h3>{imageSrc.node.title}</h3>
                                <p>{imageSrc.node.date}</p>
                            </div>
                        </Link>
                    </div>
                </BlogSlide> 
...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            queryContent: allWpCareerPost(sort: {fields: date, order: DESC}) {
                edges {
                    node {
                        title
                        slug
      ...
      `}
      render={data => <CareerScrollingBlog data={data} {...props} />}
    />
  );