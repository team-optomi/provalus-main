...
class HeroesScrollingImages extends Component {
...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            workPost: allWpScrollingImage(filter: {categories: {nodes: {elemMatch: {databaseId: {eq: 593}}}}}, sort: {fields: date, order: DESC}) {
                edges {
                    node {
                        title
                        featuredImage {
      ...
      `}
      render={data => <HeroesScrollingImages data={data} {...props} />}
    />
  );