...
class RunCaseStudySlider extends Component {
...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            allWpCaseStudy(filter: {categories: {nodes: {elemMatch: {databaseId: {eq: 517}}}}}, sort: {fields: date, order: DESC}) {
                edges {
                    node {
                      title
                      slug
      ...
      `}
      render={data => <RunCaseStudySlider data={data} {...props} />}
    />
  );