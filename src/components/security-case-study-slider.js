...
class SecurityCaseStudySlider extends Component {
...
export default props => (
    <StaticQuery
      query={graphql`
        query {
            allWpCaseStudy(filter: {categories: {nodes: {elemMatch: {databaseId: {eq: 508}}}}}, sort: {fields: date, order: DESC}) {
                edges {
                    node {
                      title
                      slug
      ...
      `}
      render={data => <SecurityCaseStudySlider data={data} {...props} />}
    />
  );